...
public:
    ...
    void add(const ElType &el)
    {
        ...
        seq_lock_.store(end, std::memory_order_release); // lock current written element
        buffer_[end] = el;
        seq_lock_.store((end + 1) % cap_, std::memory_order_release); // lock next to be written in adv
    }
...
private:
    ...
    {
        el = buffer_[idx];
        bool is_locked = seq_lock_.load(std::memory_order_acquire) == idx;
...
private:
    ...
    std::atomic<uint32_t> seq_lock_{UINT32_MAX};
    ElType *buffer_;
    size_t cap_;
};
