...
public:
    StampedRingBuffer(size_t cap) : cap_(cap)
    {
        ...
        seq_lock_ = new std::atomic<uint8_t>[cap_];
        for (size_t i = 0; i < cap_; ++i)
            seq_lock_[i].store(0, std::memory_order_relaxed);
    }
    ...
    void add(const ElType &el)
    {
        ...
        seq_lock_[end].store(1, std::memory_order_acquire); // mark as being written
        buffer_[end] = el;
        seq_lock_[end].store(0, std::memory_order_release); // mark as stable
    }
...
private:
    ...
    {
        el = buffer_[idx];
        bool is_locked = seq_lock_[idx].load(std::memory_order_acquire);
...
private:
    ...
    std::atomic<uint8_t> *seq_lock_{nullptr};
    ElType *buffer_;
    size_t cap_;
};
